...
from automl.distributed import is_enabled, get_global_rank, get_local_rank
...
def enable(log_level: LogLevel = LogLevel.INFO):
    ...
    def print(*args, **kwargs):
        ...
        if log_level.value >= LOG_LEVEL.value or force:
            # Concatenate all values to printed to avoid interleaving with other processes
    ...
    tqdm.tqdm.__init__ = lambda self, *args, **kwargs: __init__(
        self,
        position=get_global_rank() if is_enabled() else 0,
        *args,
        desc=f"{log_prefix()} {kwargs.pop('desc', '')}",
        **kwargs,
    )
